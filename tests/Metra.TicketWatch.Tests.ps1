    ...
    It 'E1: evidence-driven vocabulary proposals (corpus DF + acronym gate)' {
        InModuleScope Metra {
            ...
            $vendorOnce = @(Get-MetraTicketWatchSuggestedVocabulary `
                -Subject 'VendorX one-off payroll' `
                -CueList @('pharos') `
                -CorpusStats $corpus)
            @($vendorOnce | ForEach-Object { $_.Token }) | Should -Not -Contain 'VendorX'
            ...
            $emptyVendor = @(Get-MetraTicketWatchSuggestedVocabulary `
                -Subject 'VendorX payroll glitch' `
                -CueList @() `
                -CorpusStats $empty)
            @($emptyVendor | ForEach-Object { $_.Token }) | Should -Not -Contain 'VendorX'
