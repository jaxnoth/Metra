...
This skill analyzes OpenAPI specs and optional Postman workspace specs. It does **not** route portfolio work, pick a git project, or write tickets. For "which repo is this API in?", stay on Metra (`.\metra.ps1 routing` / `ctx`).
...
Most APIs are built for human developers who can read docs, interpret ambiguous errors, and make judgment calls. AI agents need explicit metadata, structured errors, and predictable patterns.

## The 8 Pillars

Agent-readiness is measured across 8 pillars with 48 total checks. See [pillars.md](pillars.md) for the complete reference.
...
Suggest running the readiness scan when:
...
Prefer the **api-readiness-scan** skill for the full workflow.
...
Pillar definitions adapted from the Postman Cursor plugin (`agent-ready-apis` skill, Apache-2.0). Full Postman lifecycle commands remain in the marketplace **postman** plugin if installed.
