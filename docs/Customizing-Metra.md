...
Operator-private operational why-we-chose memory - not product policy and not collaboration rhythm. Ledger: gitignored `docs/decision-registry.json`. Flow: `note` / `harvest` -> `promote` (requires `why`, `confidence`, `evidence`). Cap 50 active confirmed. Retrieved via `.\metra.ps1 decisions search|get` and `ctx -Query` (top 3) - never an always-on rule. Boundary test: would every Metra clone benefit? If yes, use `docs/Decisions.md` instead.
