...
**Last oriented:** 2026-09-02 (Yarn Phase B Atlas intake; Phase C AutoProgram leftovers)  
**Stable idea:** Metra routes. Atlas stores. Notion persists. **Yarn** prepares intake. **Loom** executes governed delivery. Vectors accelerate retrieval later - they never become truth.
...
| **Yarn** | L1.5 intake (Capture / Future-Dev / Atlas → ranked backlog → plan pack → human approval) | Metra-hosted module; `%LOCALAPPDATA%\Metra\yarn\`; [playbooks/yarn.md](playbooks/yarn.md) |
| **Loom** | Governed plan execution (queue, journal, review, operator acceptance) | Metra-hosted module `modules/Loom/`; `%LOCALAPPDATA%\Metra\loom\`; [loom-product-boundary.plan.md](loom-product-boundary.plan.md) |
