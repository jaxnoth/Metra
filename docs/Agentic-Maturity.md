...
| Evidence (current) | Playbook in `OrgBrand.Jitterbit/README.md` + project `AGENTS.md`: `Find-JitterbitStuckOperation` -> `Get-JitterbitAgentHealth` -> `Stop-JitterbitOperation` -> TranDb get/clear with `-WhatIf`; CredSSP via CredentialHelper |
