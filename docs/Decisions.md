...
## 2026-08-27 - OrgBrand DNSFilter Tailscale campus hosts pin

- Decision: On OrgBrand campus, Metra ships `.\metra.ps1 tailscale campus-hosts` to pin `login.tailscale.com` and `controlplane.tailscale.com` to Tailscale coordination anycast (`192.200.0.0/24`) in the Windows hosts file (managed marker block). Campus DNSFilter MITMs `login.tailscale.com` (DNSFilter Root CA + HSTS), which blocks Tailscale Serve enable and admin HTTPS. Hosts writes require elevation. This does not restore org Entra Tailscale enrollment and does not replace Tailscale URL ACL / Serve setup for Ops reach.
...
- Why: A hardcoded OrgBrand product list in TicketWatch.ps1 drifted (ILLiad hand-add) and duplicated solutions + registry vocabulary ownership.
