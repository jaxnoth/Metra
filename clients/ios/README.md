...
Operator enters Ops **HTTPS** MagicDNS/Serve URL (e.g. `https://jumpbox.lab.example.ts.net`). Stored in `@AppStorage`. Plain `http` is rejected before send (matches ATS).
...
1. Unlock **DevicePhone**, plug into the OrgBrand Mac (or stay paired wirelessly), tap **Trust** if asked.
...
5. Destination menu (toolbar): pick **DevicePhone** (not a simulator).
...
7. In the app gear: set Ops URL (same HTTPS MagicDNS that works in Safari on the phone, e.g. `https://jumpbox.lab.example.ts.net`). Ensure **Tailscale** is up on the phone.
